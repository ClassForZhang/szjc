 ...
 * 
 */
public class UsersRealm extends CasRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		if (authorizationInfo == null) {
			...
			//List role = roleDao.getRoles(account);
			authorizationInfo.addRoles(roleDao.getRoles(account));
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) {
		...
		UserEntity user = userDao.findUserByUserName(account);
			
		SecurityUtils.getSubject().getSession().setAttribute("activeUser", user);
