...
public class JsonUtil {
	...
	public static Map<String, String> decodeJSONtoMap(String json) {
		...
		if (json.startsWith("{") && json.endsWith("}")) {
			...
			for (String node : allNodes) {
				...
				String key = nd[0].trim();
				String value = (nd.length == 4) ? (nd[1] + ":" + nd[2] + ":" + nd[3]) : nd[1];// 处理这两类情况：1、“2013-02-27
																								// 10:43:24”；2、；http://127.0.0.1:8080/nms/index.do
	...
	/**
	 * json(json每个元素都是一个新的json)转换为jsonToJsonArray(array的每一个元素都是一个json) added by
	 * LuoChao 2013.2.28
	 */
		...
		try {
			...
			if (jsonList.length > 1) {
				for (int i = 0; i < jsonList.length; i++) {
					if (i == 0) {
						jsonList[i] = jsonList[i].substring(1, jsonList[i].length());
						jsonList[i] = jsonList[i] + "}";
					} else if (i == (jsonList.length - 1)) {
						jsonList[i] = jsonList[i].substring(0, jsonList[i].length() - 1);
						jsonList[i] = "{" + jsonList[i];
				...
				jsonList[0] = jsonList[0].substring(1, jsonList[0].length() - 1);
			}
			return jsonList;
		} catch (Exception e) {
			e.printStackTrace();
